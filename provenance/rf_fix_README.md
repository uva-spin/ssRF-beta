# Real-time ss-RF: tensor designer — RF playback correction

This package extends `spin1_ssrf_realtime_ideal_profiles_boltzmann`.
The working population model, Boltzmann recovery channels, ideal per-bin RF
scheduler, manual pulse editor, two live plots and arrow-only numeric fields
are retained. RF button orchestration has been corrected; see PLAYBACK_FIX.md. **No RF, DNP, T1, or recovery equation has been changed.**

New functionality:

* Versioned material JSON: load, save, save-as and explicitly edit **all** model,
  numerical, display and tensor-search parameters, including backend-only fields.
* Independent current-population snapshots: save a manipulated state without
  replacing it with a Boltzmann state inferred from P.
* A state-dependent tensor program generator: simultaneous constant powers,
  individually chosen for each candidate bin, all starting at zero and ending
  at one shared duration T. Q is evaluated immediately at that endpoint.
* Prediction preview, JSON/CSV pulse-program export and source-state/material
  capture. Generating or loading a program never starts RF automatically.
...
`gui.py` retains the parent layout and now delegates start/mute/replay and endpoint handling to `playback.py`.
